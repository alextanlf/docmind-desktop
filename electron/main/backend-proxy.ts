import { z } from "zod";
import { EventEnvelopeSchema, type EventEnvelope, type StreamSubscription, ErrorEnvelopeSchema } from "../shared/contracts";
...
export class DocMindClientError extends Error {
  ...
  toJSON() {
    return { code: this.code, message: this.message, retryable: this.retryable, action: this.action };
  }
}
...
const STREAM_ROUTE = /^\/api\/(?:imports\/[0-9a-f-]{36}\/events|sessions\/[0-9a-f-]{36}\/messages\/stream)$/;
...
export class BackendProxy {
  ...
  constructor(opts: { request?: RequestFn; backendManager?: { request(path: string, init?: RequestInit): Promise<Response> }; send?: (sender: StreamSender, channel: string, payload: unknown) => void }) {
  ...
  async requestJson<T>(route: string, initOrSchema: RequestInit | z.ZodType<T> = {}, schema?: z.ZodType<T>): Promise<T> {
  ...
  openStream(options: OpenStreamOptions): StreamSubscription {
    if (!STREAM_ROUTE.test(options.route.split("?")[0])) throw new DocMindClientError("INVALID_REQUEST", "流式请求路径无效");
    if (this.active.has(options.requestId)) throw new DocMindClientError("STREAM_ACTIVE", "该请求已有活动流");
    ...
    if (isChat && sessionId) {
      ...
      if (current && this.active.has(current)) throw new DocMindClientError("CHAT_STREAM_ACTIVE", "该会话已有活动聊天流");
      this.chatSessions.set(sessionId, options.requestId);
    }
    const controller = options.controller ?? new AbortController();
    const stream: ActiveStream = { controller, sender: options.sender, sessionId, lastSequence: 0 };
    ...
    return { requestId: options.requestId, cancel: () => this.cancel(options.requestId) };
  }
  ...
  private remove(requestId: string, stream: ActiveStream): void {
    ...
    if (stream.sessionId && this.chatSessions.get(stream.sessionId) === requestId) this.chatSessions.delete(stream.sessionId);
  ...
  private async consume(requestId: string, route: string, body: unknown, stream: ActiveStream, extraHeaders?: HeadersInit): Promise<void> {
    try {
      const streamHeaders = new Headers(extraHeaders);
      streamHeaders.set("Accept", "text/event-stream");
      const init: RequestInit = { signal: stream.controller.signal, headers: streamHeaders };
      if (body !== undefined) {
        init.method = "POST";
      ...
      if (!response.body) throw new DocMindClientError("BACKEND_PROTOCOL_ERROR", "后端流式响应为空", true);
      ...
      while (true) {
        ...
        for (const frame of frames) {
          ...
          if (!event || event.requestId !== requestId && route.includes("/messages/stream")) continue;
      ...
      if (finalEvent && finalEvent.sequence > stream.lastSequence) this.send(stream.sender, streamEventChannel(requestId), finalEvent);
    } catch (error) {
      if (!(error instanceof DOMException && error.name === "AbortError") && !(error instanceof Error && error.name === "AbortError")) {
        if (this.active.has(requestId)) {
          ...
          const event: EventEnvelope = { requestId, type: "error", sequence: current.lastSequence + 1, payload: { code: error instanceof DocMindClientError ? error.code : "BACKEND_REQUEST_FAILED", message: error instanceof Error ? error.message : "后端请求失败" } };
          this.send(current.sender, streamEventChannel(requestId), event);
        }
      }
    } finally {
      const current = this.active.get(requestId);
      if (current) this.remove(requestId, current);
    }
  }
...
function isSchema(value: unknown): value is z.ZodType<unknown> {
  return Boolean(value && typeof value === "object" && typeof (value as z.ZodType).parse === "function");
}
...
export function parseSseFrame(frame: string): EventEnvelope | null {
  ...
  if (!data) return null;
  try {
    return EventEnvelopeSchema.parse(JSON.parse(data));
  } catch {
    return null;
  }
}

export async function collectSse(chunks: Iterable<string | Uint8Array> | AsyncIterable<string | Uint8Array>): Promise<EventEnvelope[]> {
  ...
  for await (const chunk of chunks) {
    ...
    for (const frame of frames) {
      const event = parseSseFrame(frame);
      if (event && (!result.length || event.sequence > result[result.length - 1].sequence)) result.push(event);
    }
  }
  buffer += decoder.decode();
