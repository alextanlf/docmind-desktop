...
describe("memory management", () => {
  it("filters memory by repository", async () => {
    ...
    render(<QueryClientProvider client={new QueryClient()}><MemoryView /></QueryClientProvider>);
    ...
    await waitFor(() => expect(list).toHaveBeenCalledWith({ repositoryIds: [repository.id], kind: undefined }));
  });
  ...
  it("does not request memory until a delayed repository option is selected", async () => {
    ...
    const list = vi.fn().mockResolvedValue({ items: [], nextCursor: null });
    installDocMindApi({ repositories: { list: vi.fn().mockReturnValue(repositories) }, memory: { list } });
    render(<QueryClientProvider client={new QueryClient()}><MemoryView /></QueryClientProvider>);
    const user = userEvent.setup();
