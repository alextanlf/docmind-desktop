...
describe("批量导入", () => {
  ...
  it("reviews selected candidates and sends the discovery version with decisions", async () => {
    ...
    await waitFor(() => expect(screen.getByRole("combobox", { name: "目标知识库" })).toHaveValue(repository.id));
  ...
  it("keeps deselected persisted candidates unchecked", async () => {
    ...
    expect(api.batches.create).toHaveBeenCalledWith({ kind: "staged_directory", sourceId: "00000000-0000-0000-0000-000000000099", repositoryId: repository.id });
  });
  ...
  it("defaults to a delayed repository and sends it with the staged directory to the API", async () => {
    ...
    const repositories = new Promise<typeof repository[]>((resolve) => { resolveRepositories = resolve; });
    const api = installDocMindApi({
    ...
    expect(api.batches.create).not.toHaveBeenCalled();
    fireEvent.click(screen.getByRole("button", { name: "选择目录" }));
    await waitFor(() => expect(api.batches.create).toHaveBeenCalledWith({
