...
it.each([false, true])("cleans settled tails without deleting pending successors, failure=%s", async (fail) => {
  const coordinator = new DistillationCoordinator();
  let finish!: () => void;
  const first = coordinator.enqueue("draft", async () => {
    if (fail) throw new Error("failed");
  });
  const second = coordinator.enqueue("draft", () => new Promise<void>((resolve) => { finish = resolve; }));
...
it("cleans a rejected final operation and permits reuse", async () => {
  const coordinator = new DistillationCoordinator();
  await expect(coordinator.enqueue("draft", async () => { throw new Error("failed"); })).rejects.toThrow("failed");
  expect(coordinator.pendingCount).toBe(0);
