...
const failedBatch: BatchImport = {
  ...
  sourceKind: "staged_directory", state: "completed_with_errors", discoveryVersion: 1,
  totalCount: 2, selectedCount: 2, completedCount: 1, failedCount: 1,
  skippedCount: 0, progress: 100, message: "部分失败", errorCode: null,
  errorMessage: null, retryable: false, cancelRequested: false,
  createdAt: "2026-09-01T00:00:00Z", updatedAt: "2026-09-01T00:00:00Z",
  startedAt: null, completedAt: null, lastEventSequence: 45,
};
const failedItem: BatchItem = {
  id: "00000000-0000-0000-0000-000000000112", batchId: failedBatch.id,
  ordinal: 1, title: "b.md", displayPath: "docs/b.md", mediaType: "text/markdown",
  sizeBytes: 20, sourceRevision: "r1", allowedActions: ["create", "skip"],
  selected: true, decision: "create", state: "failed",
  importJobId: "00000000-0000-0000-0000-000000000113",
  errorCode: "IMPORT_FAILED", errorMessage: "失败", retryable: true,
};
...
it("retries persisted failed children after remount and resumes replay from the new snapshot", async () => {
  ...
  const retry = vi.fn(() => new Promise<BatchImport>((resolve) => { finishRetry = resolve; }));
  const cancel = vi.fn();
  const api = installDocMindApi({ batches: {
    get: vi.fn().mockResolvedValueOnce(failedBatch).mockResolvedValue(running),
    listItems: vi.fn().mockResolvedValueOnce({ items: [], nextCursor: "next" }).mockResolvedValue({ items: [failedItem], nextCursor: null }),
    retry,
  ...
  await waitFor(() => expect(api.batches.subscribe).toHaveBeenLastCalledWith(failedBatch.id, 47, expect.any(Function)));
  expect(cancel).toHaveBeenCalled();
  expect(screen.queryByRole("button", { name: "重试批量导入" })).not.toBeInTheDocument();
  expect(screen.getByRole("button", { name: "取消批量导入" })).toBeInTheDocument();
});

it.each(["failed", "cancelled", "completed"] as const)("does not offer child retry for terminal %s batches", async (state) => {
  const api = installDocMindApi({ batches: { get: vi.fn().mockResolvedValue({ ...failedBatch, state, retryable: true }) } });
  renderProgress();
...
it("does not retry nonretryable or unreserved children", async () => {
  const listItems = vi.fn().mockResolvedValue({ items: [{ ...failedItem, retryable: false }, { ...failedItem, id: "00000000-0000-0000-0000-000000000114", importJobId: null }], nextCursor: null });
  installDocMindApi({ batches: { get: vi.fn().mockResolvedValue(failedBatch), listItems } });
  renderProgress();
...
it("shows a failed retry and permits another attempt without an unhandled rejection", async () => {
  const api = installDocMindApi({ batches: {
    ...
    retry: vi.fn().mockRejectedValueOnce(new Error("offline")).mockResolvedValue({ ...failedBatch, state: "running" }),
  } });
  renderProgress();
