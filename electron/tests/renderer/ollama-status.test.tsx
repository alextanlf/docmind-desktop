...
const connectedMissing: OllamaStatusView = { ...unavailable, available: true, version: "0.11.0", message: "已连接" };
const emptyModels: OllamaModelsView = { available: true, models: [], checkedAt: unavailable.checkedAt, message: "没有已安装模型" };
...
describe("Ollama settings controls", () => {
  ...
  it("renders keyboard-selectable modes and local privacy copy", () => {
    installDocMindApi({ ollama: { status: vi.fn().mockResolvedValue(unavailable), models: vi.fn().mockResolvedValue(emptyModels) } });
  ...
  it("distinguishes Ollama unavailable from a missing selected model", () => {
    const { rerender } = renderWithQuery(<OllamaStatusCard status={unavailable} models={emptyModels} />);
    expect(screen.getByText("Ollama 未运行")).toBeInTheDocument();
    rerender(<QueryClientProvider client={appQueryClient}><OllamaStatusCard status={connectedMissing} models={emptyModels} /></QueryClientProvider>);
