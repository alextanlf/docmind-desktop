...
export function OllamaStatusCard({
...
}: { status?: OllamaStatusView; models?: OllamaModelsView } = {}) {
  ...
  if (suppliedStatus && !statusData?.available)
    return (
      <div role="status" className="ollama-status-card">
        <strong>Ollama 未运行</strong>
        <p>请启动本机 Ollama 服务后重新检查。</p>
      </div>
    );
  ...
  return (
    <div className="ollama-status-card" aria-live="polite">
