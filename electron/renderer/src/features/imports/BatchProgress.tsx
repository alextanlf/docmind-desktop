...
import type { BatchImport, EventEnvelope, BatchProgressPayload } from "../../../../shared/contracts";
import { appQueryClient } from "../../app/query-client";
import { clientErrorMessage } from "../settings/settings.queries";
import { batchKeys } from "./batch-import.queries";
import { useCancelBatchMutation, useContinueBatchMutation, useRetryBatchMutation } from "./batch-import.queries";
...
export function BatchProgress({ batchId }: { batchId: string }) {
  ...
  return <section aria-label="批量导入进度" className="import-progress"><div className="import-progress-title"><strong>{batch.message}</strong><span>{batch.progress}%</span></div><progress aria-label="批量导入进度" max={100} value={batch.progress} /> <p>{batch.completedCount}/{batch.selectedCount} 已完成，{batch.failedCount} 失败，{batch.skippedCount} 跳过</p>{batch.errorMessage ? <p className="editor-error" role="alert">{batch.errorMessage}</p> : null}{[cancelMutation.error, continueMutation.error, retryMutation.error].some(Boolean) ? <p className="editor-error" role="alert">操作失败，请重试</p> : null}<div className="import-progress-actions">{!isTerminal ? <button aria-label="取消批量导入" disabled={cancelMutation.isPending} className="button button-secondary" onClick={() => void cancelMutation.mutateAsync(batchId)} type="button"><Square aria-hidden="true" size={15} />取消</button> : null}{batch.state === "paused" ? <button aria-label="继续批量导入" disabled={continueMutation.isPending} className="button button-secondary" onClick={() => void continueMutation.mutateAsync(batchId)} type="button"><Play aria-hidden="true" size={15} />继续</button> : null}{batch.state === "failed" && batch.retryable ? <button aria-label="重试批量导入" disabled={retryMutation.isPending} className="button button-secondary" onClick={() => void retryMutation.mutateAsync({ batchId })} type="button"><RotateCcw aria-hidden="true" size={15} />重试</button> : null}</div></section>;
}
